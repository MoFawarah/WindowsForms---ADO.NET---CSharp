    ...
    public class StudentController : ControllerBase
    {
        ...
        public async Task<ActionResult> DeleteStudent(int Id)
        {
            ...
            try
            {
                ...
                var student = await _studentBusinessLogic.GetStudentByID(Id);
                if(student == null)
                {
                    return NotFound($"No student with Id = {Id} was found");
                }

                if (await _studentBusinessLogic.DeleteStudent(Id))
                    return Ok($"Student with Id = {Id} deleted successfully");
                else
                    return BadRequest("An error occured while deleting the student. Please try again later");
        ...
        [HttpGet("Sum/{num1:double}/{num2:double}", Name = "Sum")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public ActionResult<double> Sum(double num1, double num2)
        {
            double sum = _studentBusinessLogic.Sum(num1, num2);
            return Ok(sum);
